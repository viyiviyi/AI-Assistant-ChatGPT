...
export class CreataMessageForUser implements IMiddleware {
  ...
  prompt = `[Write your next reply from the point of view of {{user}}, using the chat history so far as a guideline for the writing style of {{user}}. must make a response that best characterizes the {{user}}. Don't write as {{char}} or system. Don't describe actions of {{char}}.]`;
  onSendBefore = (
    chat: IChat,
    context: {
    ...
    let lastCtx: CtxItem | undefined = context.history.length ? context.history.slice(-1)[0] : undefined;
