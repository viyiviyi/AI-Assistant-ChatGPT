...
export class SlackClaude implements IAiService {
  ...
  async sendMessage({
    ...
    await this.send_message_to_channel(config.channel_id, ChatManagement.getMsgContent(msg), onMessage, msg.cloudTopicId);
  }
  history = async ({
    lastMsgCloudId,
    topicCloudId,
    ...
    topicCloudId: string;
    onMessage: (msg: string, isAiMsg: boolean, msgCloudId: string, error: boolean) => Promise<void>;
    config: InputConfig;
  }) => {
    if (!config.channel_id) return onMessage('缺少频道id，请在设置添加频道id后使用', true, '', true);
    const list = await this.getHistoryMessage(config.channel_id, topicCloudId, lastMsgCloudId, 999);
  ...
  private async send_message_to_channel(
    ...
    try {
      ...
      while (!isStop && response.trim().endsWith('_Typing…_')) {
        ...
        const message = messages.find((f) => f.user === this.tokens.slack?.claude_id);
        if (!message) {
          reties += 1;
          continue;
        }
  ...
  private async getHistoryMessage(
    ...
    const replies = await this.receive_message(channel_id, thread_ts, oldest, limit);
    if (!replies || !replies.ok) {
      return [{ text: replies.error || '获取历史记录出错', ts: '', isClaude: true }];
    }
    const messages = replies['messages'] as Array<any>;
    if (!messages) return [];
    return messages
      .filter((f) => !/Please note:|Oops! Claude was un/.test(f.text) && f.ts != oldest && f.ts != thread_ts)
      .map((v) => ({
  ...
  private async send_message(channel_id: string, text: string, thread_ts: string = ''): Promise<WebAPICallResult> {
  ...
  // 获取消息列
  private async receive_message(channel_id: string, ts: string, oldest?: string, limit: number = 2): Promise<WebAPICallResult> {
    const result = await this.client.get(`/conversations.replies?channel=${channel_id}&ts=${ts}&limit=${limit}&oldest=${oldest}`);
    return result.data;
  }
}
